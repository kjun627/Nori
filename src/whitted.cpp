...
public:
    ...
    Color3f Li(const Scene *scene, Sampler *sampler, const Ray3f &ray) const override {        
        ...
        if(bsdf->isDiffuse()){
            ...
            if(xi >= 0.95f) return Le;

            BSDFQueryRecord bRec(its.toLocal(-ray.d));
            Point2f sampler2D = sampler->next2D();
            Color3f weight = bsdf->sample(bRec, sampler2D);

            if (weight.isZero()) return Le;

            Vector3f newDir = its.toWorld(bRec.wo);
            Ray3f newRay(its.p, newDir);
            newRay.mint = Epsilon;

            return Le + (1.0f/0.95f) * weight * Li(scene, sampler, newRay);
        }
